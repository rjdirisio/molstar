...
export function BackgroundTaskProgress() {
  ...
  const [tracked, setTracked] = useState<
    OrderedMap<number, TaskManager.ProgressEvent>
  >(OrderedMap());
  ...
  return (
    <div className="msp-background-tasks">
      {tracked.valueSeq().map((e) => (
        <ProgressEntry key={e!.id} event={e!} />
      ))}
      <CanvasCommitState />
    </div>
  );
...
function CanvasCommitState() {
  ...
  return (
    <div className="msp-task-state">
      <div>
        <div>Commiting renderables... {queueSize} remaining</div>
      </div>
    </div>
  );
}
...
class ProgressEntry extends PluginUIComponent<{
  event: TaskManager.ProgressEvent;
}> {
  abort = () => {
    this.plugin.managers.task.requestAbort(
      this.props.event.progress.root.progress.taskId,
      "User Request"
    );
  };
  ...
  render() {
    ...
    const subtasks =
      subtaskCount > 0 ? <>[{subtaskCount} subtask(s)]</> : void 0;

    return (
      <div className="msp-task-state">
        <div>
          {root.progress.canAbort && (
            <IconButton svg={CancelSvg} onClick={this.abort} title="Abort" />
          )}
          <div>
